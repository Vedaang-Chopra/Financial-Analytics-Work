...
**Build:** ✅ Core agent implemented, 125 tests passing (38 Phase 1 + 42 agent + 11 amfi_disclosure + 8 parser/routing + 11 validation + 3 smoke + 8 schema/fixture/db), all Phase 1A/1B/agent docs complete
...
|---|---|---|---|
| `portfolio.py` column mapping broken for real Excel files (header=None + integer columns) | `parser/portfolio.py` | Fixed (TASK-P001 — auto-detection implemented) | Fixed |
| VLM is instantiated but `analyze_page()` never called in runner BFS main loop | `agent/runner.py` | Fixed (TASK-K004 — `analyze_page()` now wired in runner) | Fixed |
...
| VLM integration | ✅ Wired + invoked | `analyze_page()` called in runner BFS loop for low-confidence pages (TASK-K004) |
| Raw file retention | ⚠️ Partial | `retained` flag in DB but files not moved to `raw_dir` (TASK-L001) |
| NAV HTML parser | ✅ Implemented | `parse_nav_html` in `parser/nav.py` (TASK-N003) |
