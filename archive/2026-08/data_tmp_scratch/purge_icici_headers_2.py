"""Purge remaining ICICI grouping-header rows (200) — backup first, then delete,
then re-check allocation sanity."""
...
PATTERNS = [
    'securitized debt instruments', 'securitised debt instruments',
    ...
    'treps', 'reverse repo', 'cash & cash equivalents', 'net current assets',
    'total net assets', 'grand total',
]

conn = psycopg2.connect(mutual_funds_url())
cur = conn.cursor()
conds = " OR ".join(f"h.security_name ILIKE '{p}'" for p in PATTERNS)
cur.execute(f"""
    SELECT h.id, ps.id, h.security_name, h.isin, h.percentage_to_nav
    FROM portfolio_holdings h
    JOIN portfolio_snapshots ps ON ps.id = h.snapshot_id
    WHERE ps.source_url LIKE '%icicipruamc%' AND ({conds})
""")
rows = cur.fetchall()
print("rows to purge:", len(rows))

out = Path("data/tmp/backup_icici_header_rows_2_2026-08-22.csv")
...
cur.execute(f"""
    ...
    WHERE ps.id = h.snapshot_id AND ps.source_url LIKE '%icicipruamc%' AND ({conds})
""")
